...
The Regular Expression engine provided by ParserObjects has fewer features than an ordinary Regular Expression engine, such as the one provided by the `System.Text.Regex` class or the popular PCRE library. The following features are supported:
...
8. Character Classes
   ...
   3. `[^abc]` inverted classes and `[^a-c]` inverted ranges
