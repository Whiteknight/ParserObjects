...
`IParser<TInput>` defines parsers which can parse an input, but do not return an output. In addition, `IParser<TInput, TOutput>` implements `IParser<TInput>`, and so can parse without specifying output type information. This parse method variant will return an object which may contain the parsed value, but as an `object` instead of as a specific type.
...
Some parsers such as `End` do not return a meaningful value.
