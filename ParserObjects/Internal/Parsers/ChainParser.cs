    ...
    public sealed class Parser<TMiddle, TData> : IParser<TInput, TOutput>
    {
        ...
        public Result<TOutput> Parse(IParseState<TInput> state)
        {
            ...
            if (nextResult.Success)
                return Result.Ok(nextParser, nextResult.Value, initial.Consumed + nextResult.Consumed, state.Input.CurrentLocation);

            checkpoint.Rewind();
            return Result.Fail(nextParser, nextResult.ErrorMessage, state.Input.CurrentLocation);
        }
