...
namespace ParserObjects
{
    ...
    public static class ParserMatchParseExtensions
    {
        ...
        /// <param name="s"></param>
        /// <returns></returns>
        public static Result<TOutput> Parse<TOutput>(this IParser<char, TOutput> parser, string s)
            => parser.Parse(new ParseState<char>(new StringCharacterSequence(s)));

        public static Result<TOutput> Parse<TInput, TOutput>(this IParser<TInput, TOutput> parser, ISequence<TInput> input)
            => parser.Parse(new ParseState<TInput>(input));

        public static Result<object> ParseUntyped<TInput>(this IParser<TInput> parser, ISequence<TInput> input)
